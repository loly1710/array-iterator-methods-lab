...
veryOldInventors = inventors.filter((oldInventors, idx ) => {
    if (oldInventors.year >= 1500 && oldInventors.year <= 1599)
        return oldInventors;
})
...
inventorNamedAda = inventors.find((firstName) => firstName.first === "Ada");
...
// Use Array.prototype.every() to check if everyone is 19 or older
...
idx = comments.findIndex((spId => spId.id === 123523));
